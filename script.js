...
const fetchSongs = async (folderName) => {
  ...
  try {
    const response = await fetch(`https://api.github.com/repositories/867577933/contents/albums/${folderName}/`);
    ...
    links.forEach((link) => {
      if (link.href.endsWith(".mp3")) {
        ...
        const songName = decodedUrl
          .split(`https://api.github.com/repositories/867577933/contents/albums/${folderName}/`)[1]
          ?.replace(".mp3", "");
